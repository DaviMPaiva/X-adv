...
from data.dataset import (
    DetectionDataset,
    detection_collate_test,
    RCNNAnnotationTransform,
)
...
parser.add_argument(
    ...
    default="OPIXray",
    type=str,
    choices=["OPIXray", "HiXray", "XAD", "CLCXray"],
    help="Dataset name",
)
parser.add_argument("--phase", default="test", type=str, help="test phase")

parser.add_argument(
    "--batch_size", default=20, type=int, help="The size of a mini batch"
)
parser.add_argument(
...
def test_net(net, cuda, dataset, labelmap, im_size=300):
    ...
    all_gts = [[[] for _ in range(num_images)] for _ in range(len(labelmap))]
    ...
    for i, (images, labels, ids, heights, widths) in enumerate(tqdm(loader)):
        ...
        for k in range(detections.shape[0]):
            for j in range(1, detections.shape[1]):
                ...
                cls_dets = np.hstack(
                    (boxes.cpu().numpy(), scores[:, np.newaxis], labels[:, np.newaxis])
