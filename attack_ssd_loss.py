...
from layers.box_utils import match  # Adjust the import path based on your project
...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="Class index to focus the attack on (default: None, target all classes)",
)
...
parser.add_argument(
    "--num_iters", default=24, type=int, help="the number of iterations of attack"
)
parser.add_argument(
    "--save_path",
    default="../results",
...
class FocusedMultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        pos = conf_t == self.focus_class  # Positive anchors for the focus class
        num_pos = pos.sum(dim=1, keepdim=True)

        # Localization Loss (only for the focus class)
        pos_idx = pos.unsqueeze(-1).expand_as(loc_data)
        loc_p = loc_data[pos_idx].view(-1, 4)
        loc_t_masked = loc_t[pos_idx].view(-1, 4)
        loss_l = F.smooth_l1_loss(loc_p, loc_t_masked, reduction="sum")
        ...
        # Normalize by number of focus class anchors
...
if __name__ == "__main__":
    ...
    elif args.focus_class is not None:
